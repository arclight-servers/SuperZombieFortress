...
void ConVar_Init()
{
	...
	CreateConVar("sm_szf_dispenser_ammo_cooldown", "4.0", "Cooldown before client could gain ammo from dispensers.", _, true, 0.0);
	CreateConVar("sm_szf_dispenser_ammo_max", "10", "Max amount of ticks dispenser could dispense before self-destructs.", _, true, 0.0);
	CreateConVar("sm_szf_dispenser_heal_rate", "0.4", "Heal rate multiplier for survivor's dispensers.", _, true, 0.0);
	...
	CreateConVar("sm_szf_melee_ignores_teammates", "1.0", "<0/1> If enabled, melee hits will ignore teammates.", _, true, 0.0, true, 1.0);
